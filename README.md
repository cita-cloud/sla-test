...
```bash
# 拷贝的方式进行备份
kubectl exec -n $NAME_SPACE -it sla-overlord-node4-0 -c patch-op -- cloud-op backup -c /etc/cita-cloud/config/config.toml -b /backup -n /data 250000

# 或者
# 导出导入的方式进行备份
kubectl exec -n $NAME_SPACE -it sla-overlord-node4-0 -c patch-op -- cloud-op backup --export -c /etc/cita-cloud/config/config.toml -b /backup -n /data 250000
```
